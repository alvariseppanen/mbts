...
class Kitti360Dataset(Dataset):
    def __init__(self,
                 ...
                 project_scale=2,
                 output_scale=1,
                 vox_origin=(-25.6, -3.2, 0),
                 voxel_size=0.4,
                 scene_size=(51.2, 6.4, 51.2)
                 ):
        ...
        self.vox_origin = np.array([vox_origin[0],vox_origin[1],vox_origin[2]])
        self.voxel_size = voxel_size
        self.scene_size = np.array([scene_size[0], scene_size[1], scene_size[2]])
        ...
        # get image coordinates of voxels, pre compute this
        self.vox2pixlut = {}
        scale_3ds = [self.project_scale, self.output_scale]
        T_velo_2_cam = np.eye(4) # let's stay in cam coordinates
        K_input_cam_n = np.array([[0.7848782, 0,        -0.03117977],
                                  [0,         2.9391184, 0.27005080],
                                  [0,         0,                  1]]) # this is scaled to image size (192, 640)
    ...
    def load_segmentations(self, seq, img_ids, load_left, load_right, img_ids_fish=None):
        segs_p_left = []
        segs_f_left = []
        segs_p_right = []
        segs_f_right = []
        ...
        for id in img_ids:
            if load_left:
                seg = cv2.imread(os.path.join(self.data_path, "data_2d_semantics", "train", seq, "image_00", "semantic", f"{id:010d}.png"), cv2.IMREAD_UNCHANGED)
                # only one frame per sequence is missing semantic label
                if seg is None: 
                    seg = np.zeros((1408, 376)).astype(np.uint8)
                seg = cv2.resize(seg, (self.target_image_size[1], self.target_image_size[0]), interpolation=cv2.INTER_NEAREST)
                seg = self.lut[seg]
                segs_p_left += [seg]

            if load_right:
                seg = cv2.imread(os.path.join(self.data_path, "data_2d_semantics", "train", seq, "image_01", "semantic", f"{id:010d}.png"), cv2.IMREAD_UNCHANGED)
                if seg is None:
                    seg = np.zeros((1408, 376)).astype(np.uint8)
                seg = cv2.resize(seg, (self.target_image_size[1], self.target_image_size[0]), interpolation=cv2.INTER_NEAREST)
                seg = self.lut[seg]
        ...
        for id in img_ids_fish:
            if load_left:
                seg_fisheye = cv2.imread(os.path.join(self.data_path, "data_2d_semantics", "train", seq, "image_02", "semantic", f"{id:010d}.png"), cv2.IMREAD_UNCHANGED)
                seg_fisheye = cv2.resize(seg_fisheye, (self.target_image_size[1], self.target_image_size[0]), interpolation=cv2.INTER_NEAREST)
                seg_fisheye = self.lut[seg_fisheye]
                segs_f_left += [seg_fisheye]
            if load_right:
                seg_fisheye = cv2.imread(os.path.join(self.data_path, "data_2d_semantics", "train", seq, "image_03", "semantic", f"{id:010d}.png"), cv2.IMREAD_UNCHANGED)
                seg_fisheye = cv2.resize(seg_fisheye, (self.target_image_size[1], self.target_image_size[0]), interpolation=cv2.INTER_NEAREST)
                seg_fisheye = self.lut[seg_fisheye]
    ...
    def __getitem__(self, index: int):
        ...
        if self.return_segmentation:
            segs_p_left, segs_f_left, segs_p_right, segs_f_right = self.load_segmentations(sequence, img_ids, load_left, load_right, img_ids_fish=img_ids_fish)
        else:
            segs_p_left, segs_f_left, segs_p_right, segs_f_right = [],[],[],[]
        ...
        # get image coordinates of voxels
        '''ids_encoder = 0 #                                          todo: get from config
        scale_3ds = [self.project_scale, self.output_scale]
        T_velo_2_cam = np.eye(4) # let's stay in cam coordinates
        ...
        for scale_3d in scale_3ds:
            projected_pix, fov_mask, _ = vox2pix(
                ...
                self.vox_origin,
                self.voxel_size * scale_3d,
                imgs[ids_encoder].shape[2],
                imgs[ids_encoder].shape[1],
                self.scene_size
            )
            data["projected_pix_{}".format(scale_3d)] = projected_pix
            data["fov_mask_{}".format(scale_3d)] = fov_mask
        '''
        data.update(self.vox2pixlut) # pre-compute this
