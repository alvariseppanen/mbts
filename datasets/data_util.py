...
def make_datasets(config):
    ...
    if type == "KITTI_Odometry":
        ...
        train_dataset = Kitti360Dataset(
            ...
            return_bev_sem=config.get("data_bev_sem", False),
            keyframe_offset=config.get("keyframe_offset", 0),
            dilation=config.get("dilation", 1),
            ...
            project_scale=config.get("project_scale", 2),
            output_scale=config.get("output_scale", 1),
            voxel_size=config.get("voxel_size", 0.2),
            vox_origin=tuple(config.get("vox_origin", (-6.4, -1, 0))),
            scene_size=tuple(config.get("scene_size", (12.8, 3.2, 25.6)))
        )
...
def make_test_dataset(config):
    ...
    if type == "KITTI_Raw":
        ...
        test_dataset = Kitti360Dataset(
            data_path=config["data_path"],
            pose_path=config["pose_path"],
            split_path=os.path.join(config.get("split_path", None), "val_files.txt"),
            target_image_size=tuple(config.get("image_size", (192, 640))),
            frame_count=config.get("data_fc", 1),
            return_stereo=config.get("data_stereo", False),
            return_bev_sem=config.get("data_bev_sem", False),
            return_vox_sem=config.get("data_vox_sem", False),
            ...
            output_scale=config.get("output_scale", 1),
            voxel_size=config.get("voxel_size", 0.2),
            vox_origin=tuple(config.get("vox_origin", (-9.6, 0, 0))),
            scene_size=tuple(config.get("scene_size", (19.2, 1.6, 25.6)))
        )
        return test_dataset
    elif type == "RealEstate10k":
        test_dataset = RealEstate10kDataset(
