...
def main():
    ...
    enc_type = config["model_conf"]["encoder"]["type"]
    project_scale = 2
    if enc_type == "volumetric":
        project_scale = config["model_conf"]["encoder"]["project_scale"]
    ...
    with torch.no_grad():
        for idx in indices:
            ...
            # added 
            projected_pix = data["projected_pix_{}".format(project_scale)] # n, h*w, 2    (only for single input image)
            fov_mask = data["fov_mask_{}".format(project_scale)]           # n, h*w       (only for single input image)

            # Move coordinate system to input frame
            poses = torch.inverse(poses[:, :1, :, :]) @ poses

            if enc_type == "volumetric":
                net.volume_encode(images, projected_pix, fov_mask, projs, poses, ids_encoder=[0], ids_render=[0])
            else:
