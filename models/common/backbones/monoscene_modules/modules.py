...
from models.common.backbones.monoscene_modules.DDR import Bottleneck3D, Bottleneck3DwoN
...
class DensityHead(nn.Module):
    ...
    def __init__(self, inplanes, planes, nbr_classes, dilations_conv_list, beta):
        super().__init__()
    ...
    def forward(self, x_in):
        ...
        for i in range(1, len(self.conv_list)):
            #y += self.bn2[i](self.conv2[i](self.relu(self.bn1[i](self.conv1[i](x_in)))))
            y += self.conv2[i](self.relu(self.conv1[i](x_in)))
        x_in = self.activation(y + x_in)  # modified
...
class ProcesswoN(nn.Module):
    def __init__(self, feature, dilations=[1, 2, 3]):
        super(ProcesswoN, self).__init__()
        self.main = nn.Sequential(
            *[
                Bottleneck3DwoN(
                    feature,
...
class UpsamplewoN(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(UpsamplewoN, self).__init__()
        self.main = nn.Sequential(
            nn.ConvTranspose3d(
                in_channels,
                out_channels,
...
class DownsamplewoN(nn.Module):
    def __init__(self, feature, expansion=8):
        super(DownsamplewoN, self).__init__()
        self.main = Bottleneck3DwoN(
            feature,
            feature // 4,
            expansion=expansion,
            stride=2,
