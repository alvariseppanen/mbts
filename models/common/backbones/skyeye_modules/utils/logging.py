...
def log_iter(mode, meters, time_meters, results, metrics, batch=True, **kwargs):
    ...
    if results is not None:
        wt_keys = list(results.keys())
        wt_keys.sort()
        for meter_key in wt_keys:
            if meter_key.startswith('wt'):
                log_key = "{}_loss".format(meter_key)
                log_value = results[meter_key]
                log_entries.append((log_key, log_value))
...
def log_wandb(wandb_summary, mode, meters, time_meters, results, metrics, batch, **kwargs):
    assert mode in ['train', 'val', 'test'], "Mode must be either 'train', 'val', or 'test'!"
    ...
    if results is not None:
        for weight_key in results.keys():
            if weight_key.startswith('wt'):
                log_key = "weight/{}_loss".format("_".join(weight_key.split("_")[1:]))
                log_value = results[weight_key]
                log_dict[log_key] = log_value
...
def accumulate_wandb_images(key, vis_dict, wandb_vis_dict, image_names, max_vis_count, **varargs):
    if key in vis_dict.keys():
        ...
        elif key == "fv_rgb_pred":
            caption_prefix = "RGB Pred"
            wandb_vis_dict = make_wandb_entry(key, caption_prefix, vis_dict, wandb_vis_dict, image_names, max_vis_count,
                                              count_key, wandb_key)

        elif key == "fv_depth_pred":
            caption_prefix = "Depth Pred"
            wandb_vis_dict = make_wandb_entry(key, caption_prefix, vis_dict, wandb_vis_dict, image_names, max_vis_count,
                                              count_key, wandb_key)

        elif key == "bev_ht":
            caption_prefix = "BEV Height"
            wandb_vis_dict = make_wandb_entry(key, caption_prefix, vis_dict, wandb_vis_dict, image_names, max_vis_count,
                                              count_key, wandb_key)
