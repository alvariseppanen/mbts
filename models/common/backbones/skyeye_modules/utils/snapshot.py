...
def pre_train_from_snapshots(model, snapshots, modules, rank):
    for snapshot in snapshots:
        if ":" in snapshot:
            current_model_module_name, saved_model_module_name, snapshot = snapshot.split(":")
        else:
            current_model_module_name, saved_model_module_name,  = None, None

        snapshot = torch.load(snapshot, map_location="cpu")
        
        if saved_model_module_name is None:
            _load_pretraining_dict(getattr(model, 'body'), snapshot)
        else:
            if saved_model_module_name in modules:
                state_dict = snapshot['state_dict']
                if rank == 0:
                    print("Loading {} layers from {} layers...".format(current_model_module_name, saved_model_module_name))
                _load_pretraining_dict(getattr(model, current_model_module_name), state_dict[saved_model_module_name])
            else:
                raise ValueError("Unrecognized network module {}".format(saved_model_module_name))
