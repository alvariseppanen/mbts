...
class SemanticSegLoss:
    ...
    def __call__(self, sem_logits, sem, weights_msk, intrinsics, dyn_obj_msk=None, depth_weight_map=None):
        sem_loss = []
        for i, (sem_logits_i, sem_i) in enumerate(zip(sem_logits, sem)):
            ...
            if dyn_obj_msk is not None:
                sem_loss_i *= dyn_obj_msk[i]

            if depth_weight_map is not None:
                sem_loss_i *= depth_weight_map[i]
...
class SemanticSegAlgo:
    ...
    @staticmethod
    def _bev_logits_and_height(head, x, intrinsics):
        sem_logits, height = head(x, intrinsics)
        return sem_logits, height

    @staticmethod
    def _logits(head, x, intrinsics):
        sem_logits = head(x)
        return sem_logits

    def compute_semantic_logits(self, head, x, intrinsics):
        bev_sem_logits, bev_height = SemanticSegAlgo._bev_logits_and_height(head, x, intrinsics)
        ...
        # packed_height = SemanticSegAlgo._pack_logits(bev_height, bev_valid_size_curr, bev_img_size_curr)
        return packed_sem_logits, bev_height


    def training_fv(self, head, x, sem, valid_size, img_size, weights_msk, intrinsics, dyn_obj_msk, depth_weight_map):
        # Compute logits and prediction
        sem_logits_ = self._logits(head, x, intrinsics)
        ...
        sem_loss = self.loss(sem_logits, sem, weights_msk, intrinsics=intrinsics, dyn_obj_msk=dyn_obj_msk,
                             depth_weight_map=depth_weight_map)
        conf_mat = self._confusion_matrix(sem_pred, sem)

        return sem_loss, conf_mat, sem_pred, sem_logits

    def inference_fv(self, head, x, valid_size, img_size, intrinsics):
        sem_logits_ = self._logits(head, x, intrinsics)
        sem_logits = self._pack_logits(sem_logits_, valid_size, img_size)
    ...
    def training_bev(self, head, x, sem_pseudo_gt, bev_weights_msk, fv_intrinsics, bev_img_size=None):
        ...
        # Compute loss and confusion matrix
        bev_sem_loss = self.loss(bev_sem_logits, sem_pseudo_gt, bev_weights_msk, intrinsics=fv_intrinsics)
    ...
    def generate_bev_using_fv(self, fv_sem_head, feat_voxel, remove_channels=None):
        conv_kernel = fv_sem_head.conv2d_sem
        feat_2d = torch.mean(feat_voxel, dim=3)
        bev_sem_logits = conv_kernel(feat_2d)
        # bev_sem_logits = F.conv2d(feat_2d, weight=conv_kernel.weight, bias=conv_kernel.bias, padding=0)
        bev_sem_logits = F.interpolate(bev_sem_logits, size=(self.bev_Z_out, self.bev_W_out), mode="bilinear", align_corners=False)
        ...
        if remove_channels is not None:
            keep_channels = [ch_idx for ch_idx in range(bev_sem_logits.shape[1])]
            for ch_idx in reversed(keep_channels):
                if ch_idx in remove_channels:
                    del keep_channels[ch_idx]
            bev_sem_logits = bev_sem_logits[:, keep_channels, :, :]

        bev_sem_pred = PackedSequence([bev_sem_logits_i.max(dim=0)[1] for bev_sem_logits_i in bev_sem_logits])

        return bev_sem_pred, bev_sem_logits
