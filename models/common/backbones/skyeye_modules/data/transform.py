...
# from po_bev_unsupervised.utils.bbx import extract_boxes
...
class BEVTransform:
    """Transform function for the BEV dataset
    ...
    centre_crop : tuple of float
        Output crop dimension. Images are cropped keeping the centre constant.
    ...
    def __call__(self, img, bev_msk=None, bev_plabel=None, fv_msk=None, bev_weights_msk=None,
            ...
            for i in range(len(fv_intrinsics)):
                if len(fv_intrinsics[i].shape) == 3:
                    ...
                    if transform_status['flip']:
                        ...
                        # fv_intrinsics[i][:, 1, 2] = self.front_resize[0] - fv_intrinsics[i][:, 1, 2]
        ...
        if bev_plabel is not None:
            ...
            # for i in range(len(bev_plabel)):
                # bev_plabel[i], bev_cat[i], bev_iscrowd[i] = self._compact_labels(bev_plabel[i], bev_cat[i], bev_iscrowd[i])
        ...
        if bev_msk is not None:
            ...
            # bev_msk = torch.cat(bev_msk, dim=0)
        if bev_plabel is not None:
        ...
        if bev_cat is not None:
            ...
            bev_bbx = None
            # bev_bbx = extract_boxes(bev_msk, bev_cat.numel())
        else:
            bev_bbx = None
        if bev_iscrowd is not None:
        ...
        if fv_cat is not None:
            ...
            # fv_bbx = [extract_boxes(msk, cat.numel()) for msk, cat in zip(fv_msk, fv_cat)]
        else:
            fv_bbx = None
        if fv_iscrowd is not None:
        ...
        if fv_intrinsics is not None:
            ...
            # fv_intrinsics = torch.cat(fv_intrinsics, dim=0)
        if ego_pose is not None:
            ego_pose = [torch.from_numpy(pose) for pose in ego_pose]
            # ego_pose = torch.cat(ego_pose, dim=0)
