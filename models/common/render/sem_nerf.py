...
class semNeRFRenderer(torch.nn.Module):
    """
    NeRF differentiable renderer
    ...
    def composite(self, model, rays, z_samp, coarse=True, sb=0):
        ...
        with profiler.record_function("renderer_composite"):
            ...
            rgbs_all, invalid_all, sigmas_all, sems_all = [], [], [], []
            if sb > 0:
            ...
            if use_viewdirs:
                ...
                for pnts, dirs in zip(split_points, split_viewdirs):
                    rgbs, invalid, sigmas, sems = model(pnts, coarse=coarse, viewdirs=dirs)
                    rgbs_all.append(rgbs)
                    invalid_all.append(invalid)
                    sigmas_all.append(sigmas)
                    sems_all.append(sems)
            else:
                for pnts in split_points:
                    rgbs, invalid, sigmas, sems = model(pnts, coarse=coarse)
                    rgbs_all.append(rgbs)
                    invalid_all.append(invalid)
                    sigmas_all.append(sigmas)
                    sems_all.append(sems)
            ...
            sem_final = torch.sum(weights.unsqueeze(-1) * sems, -2)  # (B, 3)
    ...
    def _format_outputs(
        ...
        weights, rgb_final, depth, alphas, invalid, z_samps, rgb_samps, sem_final = rendered_outputs
        n_smps = weights.shape[-1]
        out_d_rgb = rgb_final.shape[-1]
        out_d_sem = sem_final.shape[-1]
        out_d_i = invalid.shape[-1]
        if superbatch_size > 0:
            rgb_final = rgb_final.reshape(superbatch_size, -1, out_d_rgb)
            sem_final = sem_final.reshape(superbatch_size, -1, out_d_sem)
        ...
        ret_dict = DotMap(rgb=rgb_final, depth=depth, invalid=invalid, sem=sem_final)
        if want_weights:
