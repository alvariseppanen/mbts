...
class BTSWrapper(nn.Module):
    def __init__(self, renderer, config, ) -> None:
        ...
        self.enc_type = config["model_conf"]["encoder"]["type"] 
        self.project_scale = config["data"]["project_scale"]
    ...
    def forward(self, data):
        ...
        if self.enc_type == "volumetric":
            ...
            self.renderer.net.volume_encode(images, projected_pix, fov_mask, projs, poses, ids_encoder=ids_encoder, ids_render=ids_encoder, )
        else:
            self.renderer.net.compute_grid_transforms(projs[:, ids_encoder], poses[:, ids_encoder])
