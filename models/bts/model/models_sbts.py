...
class SBTSNet(torch.nn.Module):
    def __init__(self, conf):
        ...
        self.encoder_config = conf["encoder"]
        self.encoder_type = self.encoder_config["type"]
        self.encoder = make_backbone(self.encoder_config)
        self.code_xyz = PositionalEncoding.from_conf(conf["code"], d_in=3)

        if self.encoder_type == "volumetric": 
            self.vox_origin = self.encoder_config["vox_origin"]
            self.scene_size = self.encoder_config["scene_size"]
        ...
        self.n_classes = conf["mlp_class"]["n_classes"]
        ...
        self._scale = 0
        self._semantic = 4 # idx of the semantic feature tensor
    ...
    def volume_encode(self, images, projected_pix, fov_mask, Ks, poses_c2w, ids_encoder=0, ids_render=None, images_alt=None, combine_ids=None):
        ...
        volumetric_encodings = self.encoder(images_encoder.view(n * nv, c, h, w), projected_pix, fov_mask) # in cam coordinates [n*nv, c, voxel_x, voxel_y, voxel_z]

        if do_flip:
            volumetric_encodings = torch.flip(volumetric_encodings, dims=(2, ))
        
        volumetric_encodings = volumetric_encodings.view(n, nv, volumetric_encodings.shape[-4], volumetric_encodings.shape[-3], volumetric_encodings.shape[-2], volumetric_encodings.shape[-1])
        
        self.grid_f_features = volumetric_encodings
    ...
    def volume_sample_features(self, xyz, use_single_featuremap=True):
        
        n, n_pts, _ = xyz.shape
        n, nv, c, h, w, d = self.grid_f_features.shape
        ...
        xyz_cam = ((self.grid_f_poses_w2c[:, :nv, :3, :]) @ xyz.permute(0, 1, 3, 2))
        distance = torch.norm(xyz_cam, dim=-2).unsqueeze(-1)
        xyz_projected = (self.grid_f_Ks[:, :nv] @ xyz_cam).permute(0, 1, 3, 2)
        xy = xyz_projected[:, :, :, [0, 1]]
        z = xyz_projected[:, :, :, 2:3]

        xyz_cam = xyz_cam.permute(0, 1, 3, 2)
        
        # normalize zyx_cam so that points at the edges of the voxel grid are -1 and 1, could be incorrect
        vox_origin = torch.tensor([self.vox_origin[0], self.vox_origin[1], self.vox_origin[2]]).cuda()
        scene_size = torch.tensor([self.scene_size[0], self.scene_size[1], self.scene_size[2]]).cuda()
        xyz_cam = xyz_cam - vox_origin[None, None, None, :]
        xyz_cam = xyz_cam - (scene_size[None, None, None, :] / 2)
        xyz_cam = xyz_cam / (scene_size[None, None, None, :] / 2)
        
        xy = xy / z.clamp_min(EPS)
        
        invalid = (z <= EPS) | (xy[:, :, :, :1] < -1) | (xy[:, :, :, :1] > 1) | (xy[:, :, :, 1:2] < -1) | (xy[:, :, :, 1:2] > 1) | (xyz_cam[:, :, :, :1] < -1) | (xyz_cam[:, :, :, :1] > 1) | (xyz_cam[:, :, :, 1:2] < -1) | (xyz_cam[:, :, :, 1:2] > 1) | (xyz_cam[:, :, :, 2:3] < -1) | (xyz_cam[:, :, :, 2:3] > 1)
        #invalid = (z <= EPS) | (xy[:, :, :, :1] < -1) | (xy[:, :, :, :1] > 1) | (xy[:, :, :, 1:2] < -1) | (xy[:, :, :, 1:2] > 1) 
        #print(torch.count_nonzero(invalid))
        ...
        if self.learn_empty: # false
            empty_feature_expanded = self.empty_feature.view(1, 1, 1, c).expand(n, nv, n_pts, c)

        xyz_cam = xyz_cam.unsqueeze(dim=1)
        sampled_features = F.grid_sample(feature_map.view(n * nv, c, h, w, d), xyz_cam.view(n * nv, 1, 1, -1, 3), mode="bilinear", padding_mode="border", align_corners=False).view(n, nv, c, n_pts).permute(0, 1, 3, 2)
    ...
    def sample_features(self, xyz, use_single_featuremap=True):
        ...
        feature_map = self.grid_f_features[self._scale][:, :nv]
        semantic_feature_map = self.grid_f_features[self._semantic][:, :nv]
        # These samples are from different scales
        ...
        sampled_features = torch.cat((sampled_features, xyz_code), dim=-1)
        semantic_sampled_features = torch.cat((semantic_sampled_features, xyz_code), dim=-1)

        # If there are multiple frames with predictions, reduce them.
        # TODO: Technically, this implementations should be improved if we use multiple frames.
        # The reduction should only happen after we perform the unprojection.
        
        if self.grid_f_combine is not None: # None
            invalid_groups = []
            sampled_features_groups = []
        ...
        if use_single_featuremap: # True
            sampled_features = sampled_features.mean(dim=1)
            semantic_sampled_features = semantic_sampled_features.mean(dim=1)
            invalid = torch.any(invalid, dim=1)
        
        return sampled_features, invalid, semantic_sampled_features
    ...
    def forward(self, xyz, coarse=True, viewdirs=None, far=False, only_density=False):
        ...
        with profiler.record_function("model_inference"):
            ...
            if self.encoder_type == "volumetric":
                sampled_features, invalid_features = self.volume_sample_features(xyz, use_single_featuremap=not only_density) # invalid features (n, n_pts, 1)
                sampled_features = sampled_features.reshape(n * n_pts, -1)
                
                mlp_input = sampled_features.view(n, n_pts, -1)
            else:
                sampled_features, invalid_features, semantic_sampled_features = self.sample_features(xyz, use_single_featuremap=not only_density)        # invalid features (n, n_pts, 1)
                sampled_features = sampled_features.reshape(n * n_pts, -1)
                semantic_sampled_features = semantic_sampled_features.reshape(n * n_pts, -1)

                mlp_input = sampled_features.view(n, n_pts, -1)
                semantic_mlp_input = semantic_sampled_features.view(n, n_pts, -1)
            ...
            #print("mlp: ", time.time() - st) # 42*~0.0002=10ms
            ...
            if self.sample_color:
                ...
                pred_sem = F.softmax(semantic_mlp_output, dim=2)
            else:
                sigma = mlp_output[..., :1]
                sigma = F.relu(sigma)
