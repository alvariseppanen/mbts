...
class PatchRaySampler(RaySampler):
    ...
    def sample(self, images, poses, projs):
        ...
        for n_ in range(n):
            ...
            for v_, y, x in zip(patch_coords_v, patch_coords_y, patch_coords_x):
                rgb_gt_patch = images[n_][v_, y:y+self.patch_size_y, x:x+self.patch_size_x, :].reshape(-1, c)
    ...
    def reconstruct(self, render_dict, channels=None):
        ...
        c_depth = coarse["depth"]
        c_invalid = coarse["invalid"]
        c_sem = coarse.get("sem", None) # n, n_pts, n_classes
        if c_sem is not None: n_classes = c_sem.shape[-1]

        f_rgb = fine["rgb"]  # n, n_pts, v * 3
        f_weights = fine["weights"]
        f_depth = fine["depth"]
        f_invalid = fine["invalid"]
        f_sem = fine.get("sem", None) # n, n_pts, n_classes
        ...
        if c_sem is not None: coarse["sem"] = c_sem.view(n, self._patch_count, self.patch_size_y, self.patch_size_x, n_classes)
        ...
        if f_sem is not None: fine["sem"] = f_sem.view(n, self._patch_count, self.patch_size_y, self.patch_size_x, n_classes)
        ...
        render_dict["coarse"] = coarse
        render_dict["fine"] = fine
        if c_sem is not None: channels += 1
...
class ImageRaySampler(RaySampler):
    ...
    def sample(self, images, poses, projs):
        ...
        for n_ in range(n):
            ...
            if images is not None:
                rgb_gt = images[n_].view(-1, c, self.height, self.width)
    ...
    def reconstruct(self, render_dict, channels=None):
        ...
        if c_sem is not None: coarse["sem"] = c_sem.view(n, v_in, self.height, self.width, v_render, n_classes)
        ...
        if f_sem is not None: fine["sem"] = f_sem.view(n, v_in, self.height, self.width, v_render, n_classes)
