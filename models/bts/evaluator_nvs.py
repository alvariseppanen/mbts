...
class BTSWrapper(nn.Module):
    def __init__(self, renderer, config, ) -> None:
        ...
        self.eval_resolution = list(config.get("eval_resolution", None))
    ...
    def forward(self, data):
        ...
        if self.eval_resolution is not None:
            images_resized = F.interpolate(images.view(n * v, c, h, w), size=self.eval_resolution).view(n, v, c, *self.eval_resolution)
        else:
        ...
        self.renderer.net.encode(images_resized, projs, poses, ids_encoder=ids_encoder, ids_render=ids_encoder, images_alt=images * .5 + .5)
    ...
    def compute_nvs_metrics(self, data):
        ...
        imgs_gt = F.interpolate(imgs_gt, self.eval_resolution)
        imgs_pred = F.interpolate(imgs_pred, self.eval_resolution)
        ...
        lpips_score = self.lpips(imgs_pred, imgs_gt, normalize=True).mean()
